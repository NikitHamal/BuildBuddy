...
class JavaAstEditor @Inject constructor() {
    ...
    private fun applyOperation(cu: CompilationUnit, op: AgentEditOperation) {
        when (op.kind) {
            ...
            "java_remove_import" -> cu.imports.removeIf { it.nameAsString == op.payload || it.nameAsString.endsWith(op.payload) }
    ...
    private fun replaceOrUpsertMethod(cu: CompilationUnit, op: AgentEditOperation) {
        val methodDecl = parser.parseBodyDeclaration(op.payload).result.orElseThrow {
            IllegalArgumentException("Invalid Java method payload")
        ...
        val targetType = cu.types.firstOrNull { className == null || it.nameAsString == className }
        ...
        if (existing != null) {
            ...
            targetType.members = NodeList.nodeList<BodyDeclaration<*>>().apply {
                addAll(targetType.members)
                add(methodDecl)
            }
        }
    }
