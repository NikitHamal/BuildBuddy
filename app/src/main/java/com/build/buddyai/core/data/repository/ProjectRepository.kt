...
class ProjectRepository @Inject constructor(
    ...
    suspend fun createProject(project: Project): Project {
        val projectDir = File(FileUtils.getProjectsDir(context), project.id)
        projectDir.mkdirs()
    ...
    suspend fun duplicateProject(projectId: String): Project? {
        return withContext(Dispatchers.IO) {
            ...
            File(original.projectPath).copyRecursively(newDir)
    ...
    suspend fun exportProject(projectId: String): File? {
        return withContext(Dispatchers.IO) {
    ...
    suspend fun importProject(zipFile: File, name: String, packageName: String): Project {
        return withContext(Dispatchers.IO) {
            val newId = UUID.randomUUID().toString()
    ...
    fun getProjectDir(projectId: String): File {
        return File(FileUtils.getProjectsDir(context), projectId)
    }
}
